...
class LIS3MDL():
    ...
    def enableLIS(self):
        ...
        self.i2c.writeto_mem(LIS3MDL_ADDR, self.LIS_CTRL_REG1, 0x00)
        self.i2c.writeto_mem(LIS3MDL_ADDR, self.LIS_CTRL_REG3, 0x03)
        ...
        self.i2c.writeto_mem(LIS3MDL_ADDR, self.LIS_CTRL_REG3, 0x00)
        ...
        # CTRL_REG2
        # +/- 4 gauss full scale
        self.i2c.writeto_mem(LIS3MDL_ADDR, self.LIS_CTRL_REG2, 0x00);

        # CTRL_REG4
        # Ultra-high-performance mode for Z
        # 00001100b
        self.i2c.writeto_mem(LIS3MDL_ADDR, self.LIS_CTRL_REG4, 0x0c);

        self.magEnabled = True

        # Write calculated value to the CTRL_REG1 register
        self.i2c.writeto_mem(LIS3MDL_ADDR, self.LIS_CTRL_REG1, ctrl_reg1)
    ...
    def getMagnetometerRaw(self):
        ...
        x = int.from_bytes(self.i2c.readfrom_mem(LIS3MDL_ADDR, self.LIS_OUT_X_L, 2), 'big', True)
        y = int.from_bytes(self.i2c.readfrom_mem(LIS3MDL_ADDR, self.LIS_OUT_Y_L, 2), 'big', True)
        z = int.from_bytes(self.i2c.readfrom_mem(LIS3MDL_ADDR, self.LIS_OUT_Z_L, 2), 'big', True)
        data = (x, y, z)
        return data
